...
const ForgotPassword = ({ history }) => {
	...
	const resetPassword = async (e) => {
		...
		try {
			...
			setLoading(false);
			setErrMsg(error.response && error.response.data.message ? error.response.data.message : error.message);
		}
	};

	useEffect(() => {
		if (isLoggedIn || (userInfo && userInfo.user)) history.push('/campgrounds');
		...
		<FormContainer>
			...
			{!emailChecked ? (
				<Form onSubmit={resetPassword}>
					...
					<Button block type="submit" color="primary" disabled={email.length < 3}>
						Send Email
					</Button>
				</Form>
			) : (
