...
module.exports.status = asyncHandler(async (req, res, next) => {
  const user = req.user;
  let userNotifications = await User.findById(req.user._id).populate(
    "notifications",
    null,
    { isRead: false }
  );
...
module.exports.profile = asyncHandler(async (req, res) => {
  let user = await User.findOne(
    ...
    "username email profilePic description"
  );
  if (!user) {
    res.status(404);
...
module.exports.getFullprofileInfo = asyncHandler(async (req, res) => {
  ...
  const followers = await Follower.find({ following: user }).populate(
    "follower",
    "username email profilePic"
  );
  const following = await Follower.find({ follower: user }).populate(
    "following",
    "username email profilePic"
  );
  const data = { user, campgrounds, followers, following };
